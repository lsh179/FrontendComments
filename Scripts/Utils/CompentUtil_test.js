...
function CUserid() {
     return getCookie(uidcname);
}

function Converturl(url,uid) {
    if(url.indexOf('../api/')>=0)
    {
        var nurl = url;
        if(SystemApiUrl)
        {
            nurl =url.replace("../api/",SystemApiUrl+"/api/")
        }
        if(nurl.indexOf("?")>=0)
        {
            nurl=nurl+"&"+uidcname+"="+uid+"&"+tokencname+"="+getCookie(tokencname);
        }else {
            nurl=nurl+"?"+uidcname+"="+uid+"&"+tokencname+"="+getCookie(tokencname);
        }
        return nurl;
